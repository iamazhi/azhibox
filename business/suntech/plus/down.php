...
$a_k = phpweb_auth($a_k, 'DECODE', AUTH_KEY);
...
if($mod == 'phpweb')
{
	include 'include/admin/content.class.php';
	$content = new content;
	$data = $content->get($i);
	$contentid = $i;
...
if(strpos($f, 'http://') !== FALSE || strpos($f, 'ftp://') !== FALSE || strpos($f, '://') === FALSE)
{
	...
	$a_k = urlencode(phpweb_auth("i=$i&f=$f&d=$d&s=$s&t=".TIME."&ip=".IP."&m=".$m, 'ENCODE', $phpweb_auth_key));
