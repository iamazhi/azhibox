...
function cache_common()
{
	...
	{
		if(!$r['path']) $r['path'] = $r['module'] == 'phpsin' ? '' : $r['module'].'/';
		if(!$r['url']) $r['url'] = $r['module'] == 'phpsin' ? '' : $r['module'].'/';
		$data[$r['module']] = $r;
	}
	...
	$CACHE['MODEL'] = $data;
	$data = array();
	$result = $db->query("SELECT `catid`,`module`,`contentid`,`parentid`,`arrparentid`,`child`,`arrchildid`,`type`,`menuid`,`pid`,`modelid`,`modelname`,`name`,`parentdir`,`catdir`,`url` FROM `".DB_PRE."category` WHERE 1 ORDER BY `listorder`,`catid`");
	while($r = $db->fetch_array($result))
	{
		$r['url'] = url($r['url']);
		$data[$r['catid']] = $r;
...
/**	$data = array();
	...
	$CACHE['URLRULE'] = $data;
	$data = array();
    $r = $db->get_one("SELECT `setting` FROM `".DB_PRE."module` WHERE `module`='phpsin'");
	$setting = $r['setting'];
	eval("\$PHPSIN = $setting;");
	if($PHPSIN['siteurl'] =='') $PHPSIN['siteurl'] = SITE_URL;
	$CACHE['PHPSIN'] = $PHPSIN;
...
function cache_module()
{
	...
	while($r = $db->fetch_array($result))
	{
		...
		if(!$r['url'])
		{
			$r['url'] = $r['module'] == 'phpsin' ? '' : $r['module'].'/';
