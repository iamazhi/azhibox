...
function template_compile($module, $template, $istag = 0)
{
	...
	//$r = $db->get_one("SELECT * FROM ".DB_PRE."tag WHERE tagid=3");
	...
	/**$content = ($istag || substr($template, 0, 4) == 'tag_') ? '<?php function _tag_'.$module.'_'.$template.'($data, $number, $rows, $count, $page, $pages, $setting){ global $PHPWEB,$MODULE,$M,$CATEGORY,$TYPE,$AREA,$GROUP,$MODEL,$templateid,$user_id,$username;@extract($setting);?>'.template_parse($contents, 1).'<?php } ?>' : template_parse($content);**/
...
{
	$tplfile = TPL_ROOT.TPL_NAME.'/phpsin/block/'.$blockid.'.html';
	$compiledtplfile = TPL_CACHEPATH.'phpsin_block_'.$blockid.'.tpl.php';
...
function template_parse($str, $istag = 0)
{
	...
	$str = preg_replace("/\{\/tag\}/","<?php unset(\$data); ?>",$str);
	...
	if(!$istag) $str = "<?php defined('IN_PHPJSJ') or exit('Access Denied'); ?>".$str;
	return $str;
}
...
{
	global $TAG;
    if(!isset($TAG)) $TAG = cache_read('tag.inc.php', TPL_ROOT);
	
//preg_match_all("/$page/", stripslashes($TAG[$tagname]), $matches, PREG_SET_ORDER);

	if(preg_match("/page/",$TAG[$tagname]))
	{
		 $str=isset($TAG[$tagname]) ? '<?php $data = '.$TAG[$tagname].';$pages=$data[pages];$data=$data[data];?>' : '{tag_'.$tagname.'}';
		}
	else
	{
		 $str=isset($TAG[$tagname]) ? '<?php $data = '.$TAG[$tagname].';?>' : '{tag_'.$tagname.'}';
		}
   return $str;
}
