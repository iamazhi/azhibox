...
class guestbook
{
	...
	{
		$r = $this->db->get_one("SELECT count(*) AS `count` FROM $this->table where 1 $condition");
		$number = $r["count"];
	...
	function reply($gid,$reply,$passed,$hidden,$username)
	{
		return $this->db->query("UPDATE $this->table SET reply='$reply',passed='$passed',hidden='$hidden',replyer='$username',replytime='".TIME."' WHERE gid=$gid");
	}

	function realm($ip)
	{
		require_once PHP_ROOT."include/ip_area.class.php";
		$getip = new ip_area;
		$gip = $getip->get($ip);
		return $gip;
	}
	...
	function get_face()
	{
		$face = $t = '';
		$face .= '<ul>';
		foreach(glob(PHP_ROOT.'images/face/*.gif') AS $i)
		{
