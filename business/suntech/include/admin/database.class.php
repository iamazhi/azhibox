...
class database
{
	...
	function export($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype)
	{
		...
		if(trim($tabledump))
		{
			$tabledump = "# phpweb bakfile\n# version:".PHPJSJ_VERSION."\n# time:".date('Y-m-d H:i:s')."\n# type:phpweb\n# phpweb:http://www.phpweb.com\n# --------------------------------------------------------\n\n\n".$tabledump;
			$tableid = $i;
			$filename = $tabletype.'_'.date('Ymd').'_'.$random.'_'.$fileid.'.sql';
			$altid = $fileid;
			$fileid++;
			$bakfile = PHP_ROOT.'data/bakup/'.$filename;
			if(!is_writable(PHP_ROOT.'/data/bakup/')) showmessage($this->lang['data_cannot_bak_to_server'], $forward);
			file_put_contents($bakfile, $tabledump);
			@chmod($bakfile, 0777);
			if(!EXECUTION_SQL) $filename = '分卷：'.$altid.'#';
			showmessage($this->lang['bak_file']." $filename ".$this->lang['write_success'], '?file='.$this->file.'&action='.$action.'&sizelimit='.$sizelimit.'&sqlcompat='.$sqlcompat.'&sqlcharset='.$sqlcharset.'&tableid='.$tableid.'&fileid='.$fileid.'&startfrom='.$startrow.'&random='.$random.'&dosubmit=1&tabletype='.$tabletype.'&allow='.$allow);
		}
		else
		{
		   cache_delete('bakup_tables.php');
		   showmessage($this->lang['database_bak_success'],'?file=database&action=export');
		}
	...
	function import($filename)
	{
		...
		{
			$filepath = PHP_ROOT.'data/bakup/'.$filename;
		...
		else
		{
			...
			{
				showmessage($this->lang['database_recover_success'],'?file=database&action=import');
			}
		}
	}
	...
	function executesql($operation,$sql)
	{
		...
		if($operation == 'file')
		{
	        require_once PHP_ROOT.'include/upload.class.php';
			$savepath = 'data/bakup/';
		...
		if($filenames)
		{
			if(is_array($filenames))
			{
				foreach($filenames as $filename)
				{
					...
					{
						$str = file_get_contents(PHP_ROOT.'data/bakup/'.$filename);
						$str = str_charset($from, $to, $str);
						file_put_contents(PHP_ROOT.'data/bakup/'.$to.$filename, $str);
						@chmod(PHP_ROOT.'data/bakup/'.$to.$filename, 0777);
					}
				}
			}
			else
			{
				...
				{
					$str = file_get_contents(PHP_ROOT.'data/bakup/'.$filenames);
					$str = str_charset($from, $to, $str);
					file_put_contents(PHP_ROOT.'data/bakup/'.$to.$filenames, $str);
					@chmod(PHP_ROOT.'data/bakup/'.$to.$filenames, 0777);
				}
			}
		...
		if($filenames)
		{
			if(is_array($filenames))
			{
				foreach($filenames as $filename)
				{
					if(fileext($filename)=='sql')
					{
						@unlink(PHP_ROOT.'data/bakup/'.$filename);
					}
				}
				showmessage($this->lang['operation_success'], '?file='.$this->file.'&action='.$action);
			}
			else
			{
				if(fileext($filenames)=='sql')
				{
					@unlink(PHP_ROOT.'data/bakup/'.$filenames);
					showmessage($this->lang['operation_success'], '?file='.$this->file.'&action='.$action);
				}
			}
		}
	...
	function down($filename)
	{
		...
		file_down(PHP_ROOT.'data/bakup/'.$filename);
	...
	function status()
	{
		...
		return array('phpwebtables'=>$php, 'othertables'=>$other);
	}
}
?>
