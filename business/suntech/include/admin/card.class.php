...
class card
{
	...
	function add($typeid, $cardnum, $carlength, $prefix, $endtime = '')
	{
		...
		for ($i = 0 ; $i < $number ; $i++ )
		{
            ...
            if(empty($r))
            {
			    $sql = "INSERT INTO ".DB_PRE."pay_card ( `ptypeid`, `cardid`, `inputerid`, `inputer`, `mtime`, `regip`, `point`, `amount`,`endtime`) VALUES ( '{$ptypeid}','{$cardid}','{$user_id}','{$username}','{$mtime}','{$ip}','{$row['point']}','{$row['amount']}','{$endtime}') ";
			    $this->db->query($sql);
            }
            else
            {
