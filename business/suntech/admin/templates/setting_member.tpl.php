...
<form action="" method="post">
  <table align="center" cellpadding="0" cellspacing="1" class="table_form">
    ...
    <tr>
      <th width="35%" class="align_r"><strong>允许新会员注册：</strong></td>
      <td class="align_l">
    ...
    <tr>
      <th class="align_r"><strong>注册选择模型：</strong></th>
      <td class="align_l">
      <input type='radio' name='setting[choosemodel]' value='1'  <?php if($choosemodel){ ?>checked <?php } ?>> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='setting[choosemodel]' value='0'  <?php if(!$choosemodel){ ?>checked <?php } ?>> 否</td>
    </tr>
    <tr>
      <th class="align_r"><strong>新会员注册需要邮件验证：</strong></th>
      <td class="align_l">
    ...
    <tr>
      <th class="align_r"><strong>允许前台浏览会员列表：</strong></th>
      <td class="align_l">
    ...
    <tr>
      <th class="align_r"><strong>新会员注册需要管理员审核：</strong></th>
      <td class="align_l">
    ...
    <tr>
      <th class="align_r"><strong>会员注册启用验证码功能：</strong></th>
      <td class="align_l">
    ...
    <tr>
      <th class="align_r"><strong>会员注册时进行问题答案验证：</strong></th>
      <td class="align_l">
    ...
    <tr>
      <td class="align_r"><strong>会员升级付费方式：</strong></td>
      <td class="align_l">
      <input type='radio' name='setting[paytype]' value='amount'  <?php if($paytype=='amount'){ ?>checked <?php } ?>> 金钱&nbsp;&nbsp;
	  <input type='radio' name='setting[paytype]' value='point'  <?php if($paytype=='point'){ ?>checked <?php } ?>> 积分</td>
    </tr>
    <tr>
      <td class="align_r"><strong>新会员注册默认赠送点数：</strong></td>
      <td class="align_l"><input name='setting[defualtpoint]' type='text' id='defualtpoint' value='<?=$defualtpoint?>' size='5' maxlength='5'> 点
      </td>
    </tr>
    <tr>
      <td class="align_r"><strong>新会员注册默认赠送资金：</strong></td>
      <td class="align_l"><input name='setting[defualtamount]' type='text' id='defualtamount' value='<?=$defualtamount?$defualtamount:'0.00'?>' size='5' maxlength='5' require="true" datatype="currency" msg="请输入正确的金额" msgid="err_currency"> 元<span id="err_currency"></span></td>
    </tr>
    <tr>
      <td class="align_r"><strong>会员注册协议：</strong></td>
      <td class="align_l"><textarea name='setting[reglicense]' cols='60' rows='20' id='reglicense' style="width:100%"><?=$reglicense?></textarea></td>
    </tr>
    <tr>
      <td class="align_r"><strong>保留会员名设置：</strong></td>
      <td class="align_l"><textarea name='setting[preserve]' cols='30' rows='3' id='reglicense' style="width:50%"><?=$preserve?></textarea>&nbsp;&nbsp;<font style="color:#f00">用户名之间用英文“,”隔开。</font></td>
    </tr>
    <tr>
      <td class="align_r"><strong>模块访问网址（URL）：</strong></td>
      ...
      当然，也可以为会员模块绑定其它域名(需要WEB服务器的配置)</td>
    </tr>
    <tr>
      <td class="align_r">&nbsp;</td>
      <td class="align_l"><label>
        <input type="submit" name="dosubmit" id="button" value="提交" />
