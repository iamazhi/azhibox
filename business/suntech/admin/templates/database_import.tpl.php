...
?>
  <tr bgcolor="<?=$info['bgcolor']?>" >
    ...
    <td class="align_c">
	<a href="?file=<?=$file?>&action=<?=$action?>&pre=<?=$info['pre']?>&dosubmit=1">导入</a> |
	<a href="?file=<?=$file?>&action=down&filename=<?=$info['filename']?>">下载</a>
	</td>
</tr>
<?php
	}
...
&nbsp;&nbsp;
	...
	<input type="submit" name="submit1" value=" 字符集转换 " title="转换完毕后以新字符集为前缀保存" onClick="document.myform.action='?file=<?=$file?>&action=changecharset'">
&nbsp;&nbsp;
    <input type="submit" name="submit2" value=" 删除选中的备份 " onClick="document.myform.action='?file=<?=$file?>&action=delete'">
</div>
</form>
...
?>
  <tr>
    ...
    <td class="align_c">
	<a href="?file=<?=$file?>&action=<?=$action?>&filename=<?=$other['filename']?>&dosubmit=1">导入</a> |
	<a href="?file=<?=$file?>&action=down&filename=<?=$other['filename']?>">下载</a>
	</td>
</tr>
<?php
	}
...
&nbsp;&nbsp;
   <select name="tocharset">
	...
	<input type="submit" name="submit1" value=" 字符集转换 " title="转换完毕后以新字符集为前缀保存" onClick="document.myform1.action='?file=<?=$file?>&action=changecharset'">
&nbsp;&nbsp;
    <input type="submit" name="submit2" value=" 删除选中的备份 " onClick="document.myform1.action='?file=<?=$file?>&action=delete'">
</div>
</form>
