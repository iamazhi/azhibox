...
<tbody>
    ...
    <tr> 
      ...
      <td>
	  <input name="tagname" id="tagname" type="text" size="30" value="<?=$tagname?>" require="true" datatype="require" url="?mod=<?=$mod?>&file=<?=$file?>&action=checktag" msg="标签名称必填|"><br/>
	  </td>
    </tr>
    <tr> 
      ...
      <td>
          ...
          会员模块
          <input type="radio" name="tag_config[type]" value="sql" onClick="location='?file=tag&action=add&module=member&type=sql'" <?=$type=='sql' ? 'checked' : ''?> /> 通过自定义SQL调用
	  </td>
    </tr>
</tbody>
...
<tbody >
    ...
    <td><?=form::select_model('modelid', 'modelid', '不限', $modelid, 'onchange="javascript:document.myform.action.value=\'add\';document.myform.submit()"')?></td>
    </tr>
...
<?php }?>
    ...
    <tr> 
      ...
      <td><input type="text" name="tag_config[number]" size="10" value="10"> 条 <span id="pagesize" style="display:none;">当此标签用于栏目列表分页时且生成Html时，必须与系统所设置分页数（<font color="red"><?=$PHPWEB['pagesize']?></font>）保持一致</span></td>
    </tr>
