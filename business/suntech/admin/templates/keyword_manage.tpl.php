...
?>
    <tr>
       ...
       <td class="align_c"><?=$info['hits']?></td>
        ...
        <td class="align_c"><a href="?mod=<?=$mod?>&file=<?=$file?>&action=edit&tag=<?=urlencode($info['tag'])?>">修改</a> | <a href="javascript:confirmurl('?file=<?=$file?>&action=delete&tagid=<?=$info['tagid']?>', '是否删除该关键词')">删除</a> </td>
    </tr>
<?php
	}
}
