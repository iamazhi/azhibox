...
require CLASS_PATH."strreplace.class.php";
...
switch($action)
{
	...
	case 'import':
		...
		{
			$others = array();
			$sqlfiles = glob(PHP_ROOT.'/data/bakup/*.sql');
			if(is_array($sqlfiles))
			{
				...
				{
					if(preg_match("/(phpwebtables_[0-9]{8}_[0-9a-z]{4}_)([0-9]+)\.sql/i",basename($sqlfile),$num))
					{
		...
		if($dosubmit)
		{
			...
			else
            {
                if(is_array($result) && !empty($result))
                {
                    $data = array();
                    $data = $result;
                    include tpl('database_executesql');
                }
			}
		}
		else
	...
	case 'replace':
		...
		else
		{
			...
			$referer = urlencode('?file='.$file.'&action='.$action);
			$type = '1';
