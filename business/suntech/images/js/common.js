...
{
	var arr=Dialog('?file=more_pic_select', '', 700, 500);
	if(arr!=null)
	{
		var select = '';
		$.get('?file=more_pic_select&action=getdata&currentdir='+arr, function(data){
		    if(data !=null)
			{
...
function navigation_selected(id)
{
	$('#navigation_'+id).addClass('selected');
}
...
function CutPic(textid,thumbUrl){
  ...
  var arr=Dialog(phpweb_path + 'corpandresize/ui.php?'+thumb,'',700,500);
  if(arr!=null){
    $('#'+textid).val(arr);
  }
}
