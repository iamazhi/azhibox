               ...
               <li class=unblock id="m_left"></li>
                ...
                <div id="m_right">
                  <ul class="my_nav" id="top_t">
                    <li class="sec_2"><?php if($status==0) { ?><font color="red">待付款</font>订单<?php } elseif ($status==1) { ?><font color="#FF6600">已付款</font>订单</a><?php } elseif ($status==2) { ?><font color="#0000FF">交易成功</font>订单<?php } else { ?><font color="#006600">交易关闭</font>订单</a><?php } ?></li>
                  </ul>
...
<?php $ARRAY = get("SELECT * FROM `phpjsj_order` WHERE `userid` = '$user_id'  AND `status` = '$status'", 20, $page, "", "", "","","0");$DATA=$ARRAY['data'];$total=$ARRAY['total'];$count=$ARRAY['count'];$pages=$ARRAY['pages'];unset($ARRAY);foreach($DATA AS $n=>$r){$n++;?>
<tr >
