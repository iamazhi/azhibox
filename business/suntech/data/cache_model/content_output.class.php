	...
	function downfile($field, $value)
   {
	...
	if($mode)
	{
		...
		foreach($servers AS $k=>$server)
		{
			...
			$serverurl = $server[1];
			$a_k = urlencode(phpweb_auth("i=$contentid&s=$serverurl&m=1&f=$value&d=$downloadtype", 'ENCODE', AUTH_KEY));
			$result .= "<a href='down.php?a_k=$a_k' target='_blank'>$server[0]</a>";
		}
	}
	else
	{
		$a_k = urlencode(phpweb_auth("i=$contentid&m=0&f=$value", 'ENCODE', AUTH_KEY));
...
function downfiles($field, $value)
{
	...
	foreach($values AS $k=>$v)
	{
		...
		$name = $v[0];
		$downurl = $v[1];
		$a_k = urlencode(phpweb_auth("i=$contentid&s=$serverurl&m=0&f=$downurl&d=$downloadtype", 'ENCODE', AUTH_KEY));
		$result .= "<a href='down.php?a_k=$a_k' target='_blank'>$name</a>";
	}
	return $result;
}
	function content($field, $value)//is ther
	{
	...
	function flashupload($field, $value)
	{
		...
		foreach($values AS $k=>$v)
		{
			...
			$number = $k+1;
			$a_k = urlencode(phpweb_auth("i=$contentid&m=0&p=$p&f=$videourl", 'ENCODE', AUTH_KEY));
	...
	{
		global $attachment, $contentid, $show_url_path, $mod, $C, $PHPSIN, $MODULE;
		...
		if($modelcache['ishtml'] && $this->fields[$field]['ishtml'] && $show_url_path)
		{
			...
			$head['title'] = $title.' - '.$PHPSIN['sitename'];
			$head['keywords'] = $keywords;
			$head['description'] = $this->data['title'];
			ob_start();
			foreach($images AS $page=>$v)
			{
				$filename = PHP_ROOT.$show_url_path.'_'.$page.'.'.$PHPSIN['fileext'];
	...
	{
		return '<a href= target="_blank" class="username">'.$this->data['username'].'</a>';
