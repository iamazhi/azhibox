...
class content_tag
{
    ...
    function __construct($modelid)
    {
		...
		$this->fields = $this->modelid ? cache_read($this->modelid.'_fields.inc.php', CACHE_MODEL_PATH) : cache_read('common_fields.inc.php', CACHE_MODEL_PATH);
    }
	...
	function get($fields, $where, $orderby)
	{
		...
		$array_diff = array_diff($fields, $content_table_fields);//计算数组的差集  比较字表是否相同
		$this->_diff = $array_diff;
		//echo count($content_table_fields);echo count($array_diff);
		...
		if($wheresql){$wheresql = implode(' AND ', array_filter($wheresql));}//is where
