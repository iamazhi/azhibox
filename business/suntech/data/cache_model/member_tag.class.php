...
class member_tag
{
    ...
    function __construct($modelid)
    {
		...
		$this->common_fields = cache_read('member_fields.inc.php', CACHE_MODEL_PATH);
	...
	function get($fields, $where, $orderby)
	{
		...
		if($this->model_fields)
		{
			...
			//╦Эпбн╙$tablename = "`".DB_PRE."member_cache` a, `".DB_PRE."member_info` i, `".DB_PRE."member_".$MODEL[$this->modelid]['tablename']."` b";
			$tablename = "`".DB_PRE."member` a, `".DB_PRE."member_info` i, `".DB_PRE."member_".$MODEL[$this->modelid]['tablename']."` b";
			$whereunion = 'WHERE a.userid=b.userid AND a.userid=i.userid';
		}
		else
		{
			$tablename = "`".DB_PRE."member` a `".DB_PRE."member_info` i";
