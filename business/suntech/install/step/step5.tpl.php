...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#password").formValidator({onshow:"6到20个字符<font color='FFFF00'>（默认为 phpsin）</font>",onfocus:"密码合法长度为6至20位"}).inputValidator({min:6,max:20,onerror:"密码合法长度为6至20位"});
  ...
  <div class="warp">
   ...
   <legend>填写数据库信息</legend>
			...
			<td><input name="tablepre" type="text" id="tablepre" value="<?php echo DB_PRE?>" class="input-text" />  <img src="./skin/images/help.png" style="cursor:pointer;" title="如果一个数据库安装多个phpcms，请修改表前缀" align="absmiddle" />
			<span id='helptablepre'></span></td>
			</tr>
			<tr>
			<th align="right">数据库字符集：</th>
			<td>
			<input name="dbcharset" type="radio" id="dbcharset" value="" <?php if(strtolower(DB_CHARSET)=='') echo ' checked="checked" '?>/>默认
			...
			<img src="./skin/images/help.png" style="cursor:pointer;" title="如果Mysql版本为4.0.x，则请选择默认；&#10;如果Mysql版本为4.1.x或以上，则请选择其他字符集（一般选GBK）" align="absmiddle" />
...
<?php if(DB_PCONNECT==0) echo ' checked '?>/>否
			<img src="./skin/images/help.png" style="cursor:pointer;" title="如果启用持久连接，则数据库连接上后不释放，保存一直连接状态；如果不启用，则每次请求都会重新连接数据库，使用完自动关闭连接。" align="absmiddle" /><span id='helppconnect'></span>
			<span id='helptablepre'></span></td>
			</tr>
		  </table>
   </fieldset> 
   <fieldset><legend>必选模块</legend><table width="100%" cellspacing="1" cellpadding="0">
			  <tr>
...
errmsg[0] = '您已经安装过Phpsin，系统会自动删除老数据！是否继续？';
errmsg[2] = '无法连接数据库服务器，请检查配置！';
errmsg[3] = '成功连接数据库，但是指定的数据库不存在并且无法自动创建，请先通过其他方式建立数据库！';
errmsg[6] = '数据库版本低于Mysql 4.0，无法安装Phpsin，请升级数据库版本！';
